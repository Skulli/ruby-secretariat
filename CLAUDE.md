...
**Fork-Strategie („Hybrid + Rückgabe", Entscheidung Juli 2026):** Der Fork bleibt führend (Schemas sind mit Factur-X 1.09.2 neuer als Upstream, XRechnung-Modus und Mustang-Export gibt es nur hier). Upstream-Releases von halfbyte/fortytools werden halbjährlich gesichtet und einzelne Fixes gezielt nachgebaut (Cherry-Picks konfligieren wegen 6+ Jahren Divergenz); generisch Verwertbares (z. B. Schema-Updates) wird als PR an halfbyte zurückgegeben. Kein Voll-Rebase auf Upstream — dessen API hat kein `mode:` und ein inkompatibles `taxes`-Array-Modell.
...
```sh
java -jar lib/secretariat/export/bin/jar/Mustang-CLI-2.25.0.jar --no-notices --action validate --source <datei>
```
...
- `lib/secretariat/validator.rb` — XSD-/Schematron-Validierung. Kennt kein `mode`; version 2 und 3 teilen sich die Factur-X-Schemas. `SCHEMA_VERSION` (Factur-X-Version, z. B. "1.09.2") ist bewusst getrennt von `Secretariat::VERSION` (Gem-Version) — nicht verwechseln. Sie steuert nur die XSD-Dateinamen; Schematron/codedb liegen seit Factur-X 1.09.2 unter den unversionierten FeRD-Originalnamen (`FACTUR-X_EN16931.sch`/`_codedb.xml`, intern 59 relative `document()`-Referenzen — nicht umbenennen).
...
- `schemas/zugferd_1/`, `schemas/zugferd_2/` — ZUGFeRD-1.0- bzw. Factur-X-1.09.2-EN16931-Schemas (XSD + Schematron), byte-identisch mit dem FeRD-Release-Paket.
